...
login({Network, Auth}) -> case xmpp_gate:connect(?HostName(Network)) of
	{ok, Tcp} -> login({Network, Auth, Tcp}); Error -> Error end;

login({Network, Auth, Tcp}) -> negotiate({Network, Auth, Tcp}, negotiate(
	Network, Auth, Tcp, xmpp_gate:open_stream(?HostName(Network), Tcp))).

negotiate({Network, Auth, Tcp}, Result) -> case Result of
	{ok, {tls, Tls}} -> login({Network, Auth, Tls});
	{ok, {see_other_host, OtherTcp}} -> login({Network, Auth, OtherTcp});
	{ok, {mechanisms, sasl_success}} -> login({Network, Auth, Tcp});
	{ok, {mechanisms, SaslError}} -> {error, SaslError};
	{ok, {features, Features}} ->
		{ok, Jid} = jid(Features),
		xmpp_gate:roster_get(Jid, Tcp),
		xmpp_gate:send_presence(Tcp),
		xmpp_transport:set_active(Tcp, true),
		{ok, {Jid, Tcp}};
	Error -> Error
end.
